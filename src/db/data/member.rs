...
impl Session {
    ...
    pub async fn get_member(&self, account_id: i32) -> Result<MemberInfo, Error> {
        ...
        for row in &rows {
            ...
            match houses
                .iter_mut()
                .find(|h| h.house_info.house_id == house_id)
            {
                Some(v) => v.account.push(AccountInfo {
                    account_id,
                    username,
                }),
                None => houses.push(HouseMember {
    ...
    pub async fn get_account_ids_by_house_id(&self, house_id: i32) -> Result<Vec<i32>, Error> {
        self.0
            .query(
                "SELECT account_id FROM member WHERE house_id = $1",
                &[&house_id],
            )
            .await
    ...
    pub async fn get_house_ids_by_account_id(&self, account_id: i32) -> Result<Vec<i32>, Error> {
        self.0
            .query(
                "SELECT house_id FROM member WHERE account_id = $1",
                &[&account_id],
            )
            .await
            .map(|rows| rows.into_iter().map(|row| row.get("account_id")).collect())
    ...
    pub async fn is_member(&self, account_id: i32, house_id: i32) -> bool {
        self.0
            .query_one(
                "SELECT 1 FROM member WHERE account_id = $1 AND house_id = $2",
                &[&account_id, &house_id],
            )
            .await
            .is_ok()
